...
# 1. Надёжность водителя: связано с принятием и завершением заказов
driver_reliability_fv = FeatureView(
    name="driver_reliability",
    entities=[driver],
    ttl=timedelta(hours=12),  # Надёжность может меняться быстрее
    schema=[
        Field(name="acc_rate", dtype=Float32),
        Field(name="conv_rate", dtype=Float32),
    ],
    online=True,
    source=driver_stats_source,
    tags={"domain": "driver_behavior", "use_case": "reliability_model"},
)

# 2. Активность водителя: операционная нагрузка
driver_activity_fv = FeatureView(
    name="driver_activity",
    entities=[driver],
    ttl=timedelta(days=3),  # Активность стабильнее во времени
    schema=[
        Field(name="avg_daily_trips", dtype=Int64),
    ],
    online=True,
    source=driver_stats_source,
    tags={"domain": "operations", "use_case": "demand_forecasting"},
)

# === On-Demand Feature View: Risk-Adjusted Driver Performance ===

@on_demand_feature_view(
    sources=[driver_stats_fv],  # Используем исходный FeatureView
    schema=[
        Field(
...
def driver_risk_adjusted_performance(inputs: pd.DataFrame) -> pd.DataFrame:
    # Создаём копию, чтобы не менять оригинал
    df_inputs = inputs.copy()

    # Приводим потенциально проблемные колонки к числовому типу
    df_inputs["avg_daily_trips"] = pd.to_numeric(df_inputs["avg_daily_trips"], errors="coerce").fillna(0)
    df_inputs["acc_rate"] = pd.to_numeric(df_inputs["acc_rate"], errors="coerce").fillna(0.0)
    df_inputs["conv_rate"] = pd.to_numeric(df_inputs["conv_rate"], errors="coerce").fillna(0.0)

    log_trips = np.log1p(df_inputs["avg_daily_trips"])
    reliability = (df_inputs["acc_rate"] + df_inputs["conv_rate"]) / 2.0

    df = pd.DataFrame()
    df["risk_adjusted_score"] = log_trips * reliability * (0.5 + 0.5 * reliability)

    epsilon = 1e-6
    df["engagement_level"] = 2.0 / (
        1.0 / (log_trips + epsilon) + 1.0 / (reliability + epsilon)
    )

    return df

driver_performance_realtime = FeatureService(
    name="driver_performance_realtime",
    features=[
        driver_stats_fv,  # исходные признаки
        driver_risk_adjusted_performance,  # новые on-demand признаки
    ],
)