...
# =============== ПЕРЕМЕННЫЕ KAFKA ===============
KAFKA_BOOTSTRAP_SERVERS = Variable.get("KAFKA_BOOTSTRAP_SERVERS")  # Например: "rc1a-kafka.cluster.yandex.net:9092"
KAFKA_INPUT_TOPIC = Variable.get("KAFKA_INPUT_TOPIC", default_var="fraud-transactions-input")
KAFKA_OUTPUT_TOPIC = Variable.get("KAFKA_OUTPUT_TOPIC", default_var="fraud-transactions-predictions")
...
KAFKA_SASL_USERNAME = Variable.get("KAFKA_SASL_USERNAME")
KAFKA_SASL_PASSWORD = Variable.get("KAFKA_SASL_PASSWORD")
KAFKA_CHECKPOINT_LOCATION = f"s3a://{S3_BUCKET_NAME}/kafka_checkpoints/"
...
#    )
    ...
    # Генерация данных в Kafka (полная версия с параметризацией)
    generate_kafka_data = DataprocCreatePysparkJobOperator(
        task_id="generate_kafka_data",
        main_python_file_uri=f"{S3_SRC_BUCKET}/kafka_data_producer.py",
        connection_id=YC_SA_CONNECTION.conn_id,
        dag=dag,
        args=[
            "--input-file", f"{S3_INPUT_DATA_BUCKET}/train.csv",
    ...
    # Потоковая обработка с применением модели
    streaming_inference = DataprocCreatePysparkJobOperator(
        task_id="streaming_inference",
        main_python_file_uri=f"{S3_SRC_BUCKET}/kafka_streaming_inference.py",
        connection_id=YC_SA_CONNECTION.conn_id,
        dag=dag,
        args=[
            ...
            "--checkpoint-location", f"s3a://{S3_BUCKET_NAME}/kafka_checkpoints/",
            "--security-protocol", KAFKA_SECURITY_PROTOCOL,
            "--sasl-mechanism", KAFKA_SASL_MECHANISM,
            "--sasl-username", KAFKA_SASL_USERNAME,
            "--sasl-password", KAFKA_SASL_PASSWORD,
        ...
        properties={
            ...
            'spark.yarn.appMasterEnv.PYSPARK_DRIVER_PYTHON': './.venv/bin/python3',
            'spark.yarn.appMasterEnv.PYTHONUNBUFFERED': '1',
            'spark.jars.packages': 'org.apache.spark:spark-sql-kafka-0-10_2.12:3.0.3',
        },
    )
...
#    # pylint: disable=pointless-statement
    setup_connections >> create_spark_cluster >> generate_kafka_data >> streaming_inference >> delete_spark_cluster
#    setup_connections >> create_spark_cluster >> generate_kafka_data
#    setup_connections >> create_spark_cluster >> train_model >> ab_test_validation >> generate_kafka_data >> streaming_inference >> delete_spark_cluster
